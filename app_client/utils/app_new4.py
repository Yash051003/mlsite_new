...
current_sounds = {}  # Tracks currently playing sounds
...
# Initialize gesture memory globally
gesture_memory = {"Left": None, "Right": None}

def play_sound(file):
    pygame.mixer.init()
    # Create a new channel for each sound to allow overlapping
    sound = pygame.mixer.Sound(file)
    # Play the sound without waiting for it to finish
    sound.play()
    # No waiting loop - return immediately
...
def process_gesture_output(detected_hands):
    ...
    # Extract hand and gesture information
    parts = detected_hands.split(", ")
    if len(parts) < 2:
        return

    hand_label = parts[0].split(": ")[-1]
    hand_sign_text = parts[1].split(": ")[-1]

    if hand_label in gesture_memory:
        if gesture_memory[hand_label] != hand_sign_text:  # Only update if gesture changes
            gesture_memory[hand_label] = hand_sign_text
        else:
            return  # Skip redundant processing

    time.sleep(0.5)  # Small delay to allow pairing

    if gesture_memory["Left"] and gesture_memory["Right"]:
        if gesture_memory["Left"] == "thumbindex" and gesture_memory["Right"] == "Fist":
            play_sound(sound_12)
            ...
            play_sound(sound_11)
        elif gesture_memory["Left"] == "thumb" and gesture_memory["Right"] == "thumb":
            play_sound(sound_4)
...
def draw_info_text(image, brect, handedness, hand_sign_text, finger_gesture_text):
    ...
    detected_hands = f"Detected Hand: {hand_label}, Gesture: {hand_sign_text}"
    print(detected_hands)

    # Process gesture output for sound
    process_gesture_output(detected_hands)
